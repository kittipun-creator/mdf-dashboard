...
def load_data():
    ...
    def smart_parse_date(series):
        def clean_val(val):
            ...
            # แก้ปัญหาปี 2569 และ /69 ให้กลายเป็น 2026 ในระดับข้อความ
        ...
        # ปรับแก้ปี ค.ศ. ที่เกิดจากการอ่านปีสลับย้อนหลัง
        def adjust_year(dt):
            if pd.notna(dt):
                if dt.year > 2400:
    ...
    # สำรองข้อมูล: หากช่อง 'วันที่ขัด' ว่าง ให้ดึง 'วันที่ผลิต' มาใช้แทนทันที
...
# 2. ตัวกรอง วันที่ผลิต / วันที่ขัด
    ...
    if not valid_dates.empty:
        ...
        if isinstance(date_range, (tuple, list)):
            if len(date_range) == 2:
                ...
                mask = (line_filtered_df[active_parsed_col].dt.date >= start_date) & (
                    line_filtered_df[active_parsed_col].dt.date <= end_date
                )
                filtered_df = line_filtered_df.loc[mask]
            elif len(date_range) == 1:
                start_date = date_range[0]
                mask = line_filtered_df[active_parsed_col].dt.date == start_date
                filtered_df = line_filtered_df.loc[mask]
        else:
            mask = line_filtered_df[active_parsed_col].dt.date == date_range
            filtered_df = line_filtered_df.loc[mask]
    else:
...
# ==========================================
# 4. แสดงผลหัวข้อตาม Line ที่เลือก
# ==========================================
st.title(f"🏭 ระบบวิเคราะห์ค่าความเรียบมัน Line {selected_line}")

# ==========================================
# 5. กล่องสรุปตัวเลขสำคัญ (Key Metrics)
# ==========================================
...
# ==========================================
# 6. กราฟแสดงผลการขัด (Data Visualization)
# ==========================================
st.markdown("---")
...
# ==========================================
# 7. ให้ AI วิเคราะห์ข้อมูลสด (Gemini 3.6 Flash)
# ==========================================
st.markdown("---")
